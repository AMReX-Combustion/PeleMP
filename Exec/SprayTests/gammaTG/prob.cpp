  ...
  const amrex_real* probhi)
{
  ...
  const amrex::Real mu =
    PeleC::prob_parm_device->rho0 * PeleC::prob_parm_device->v0 * refL / PeleC::prob_parm_device->reynolds;
  ...
  if (std::abs(rhoRatio - partRho/refRho) > 10.) {
    amrex::Print() << "Restart solution with particles.fuel_rho = " << refRho*rhoRatio << std::endl;
    amrex::Abort();
  }
