...
SprayParticleContainer::estTimestep (int level, Real cfl) const
{
  ...
  if (level >= this->GetParticles().size() || PeleLM::particle_mom_tran == 0)
    return dt;
                                        ...
                                        MultiFab*   u_mac)
{
  ...
  Real mass_src_fac = 0.;
  Real eng_src_fac = 0.;
  if (AMREX_SPACEDIM == 2) {mom_src_fac = 1e-3;} else if (AMREX_SPACEDIM == 3) {mom_src_fac = 1e-5;};
  if (AMREX_SPACEDIM == 2) {mass_src_fac = 1e-1;} else if (AMREX_SPACEDIM == 3) {mass_src_fac = 1e-3;};
  if (AMREX_SPACEDIM == 2) {eng_src_fac = 1e-5;} else if (AMREX_SPACEDIM == 3) {eng_src_fac = 1e-7;};
  ...
  const int rhoIndx = Density;
  const int momIndx = 0;
  const int engIndx = Density+NUM_SPECIES+1;
  const int utempIndx = engIndx+1;//Temp;
  const int specIndx = Density+1;//first_spec;
    ...
    AMREX_FOR_1D ( Np, i,
    {
      ...
      if (p.id() > 0) {
        ...
        for (int aindx = 0; aindx != AMREX_D_PICK(2, 4, 8); ++aindx) {
          ...
          Real cur_rho = statearr(cur_indx, rhoIndx)*rho_to_cgs; // pelelm
          rho_fluid += cur_coef*cur_rho; 
          Real inv_rho = 1./cur_rho;
#ifndef SPRAY_PELE_LM
          AMREX_D_TERM(Real velx = statearr(cur_indx, momIndx)*vel_to_cgs; //pelelm
                       Real ke = 0.5*velx*velx;
                       vel_fluid[0] += cur_coef*velx;,
                       Real vely = statearr(cur_indx, momIndx+1)*vel_to_cgs; // pelelm
                       ke += 0.5*vely*vely;
                       vel_fluid[1] += cur_coef*vely;,
                       Real velz = statearr(cur_indx, momIndx+2)*vel_to_cgs; // pelelm
                       ke += 0.5*velz*velz;
                       vel_fluid[2] += cur_coef*velz;);
#endif
          for (int sp = 0; sp != NUM_SPECIES; ++sp) {
            int mf_indx = sp + specIndx;
            Real cur_mf = statearr(cur_indx, mf_indx)*rho_to_cgs*inv_rho; // pelelm
            Y_fluid[sp] += cur_coef*cur_mf;
            mass_frac[sp] = cur_mf;
          }
          Real intEng = statearr(cur_indx, engIndx)*inv_rho - ke;
          Real T_val = statearr(cur_indx, utempIndx);
          //EOS::EY2T(intEng, mass_frac, T_val); // pelelm
        ...
        while (isub <= nsub) {
	      ...
	      drag_coef =
            ...
            //fluid_eng_src += S_dmu_dot_u + m_dot*part_ke; //dont do in pelelm
          ...
          if (mom_trans || mass_trans || heat_trans) {
            ...
            if (mom_trans) {
              ...
              if (do_move) {
                for (int dir = 0; dir != AMREX_SPACEDIM; ++dir) {
		  Gpu::Atomic::Add(&p.pos(dir), dt*velp[dir][i]*pos_to_mks); //pelelm
            ...
            if (mom_trans) {
              ...
              if (do_move) {
                for (int dir = 0; dir != AMREX_SPACEDIM; ++dir) {
                  Gpu::Atomic::Add(&p.pos(dir), dt*p.rdata(pstateVel+dir)*pos_to_mks); //pelelm
            ...
            for (int aindx = 0; aindx != AMREX_D_PICK(2, 4, 8); ++aindx) {
              ...
              if (mom_trans) {
                for (int dir = 0; dir != AMREX_SPACEDIM; ++dir) {
                  ...
                  Gpu::Atomic::Add(&sourcearr(cur_indx, nf), cur_coef*fluid_mom_src[dir]*mom_src_fac); //pelelm
                }
              }
              if (mass_trans) {
                Gpu::Atomic::Add(&sourcearr(cur_indx, rhoIndx), cur_coef*m_dot*mass_src_fac); //pelelm
                for (int spf = 0; spf != SPRAY_FUEL_NUM; ++spf) {
                  const int nf = specIndx + fdat.indx(spf);
                  Gpu::Atomic::Add(&sourcearr(cur_indx, nf), cur_coef*Y_dot[spf]*mass_src_fac); // pelelm
                }
              }
              Gpu::Atomic::Add(&sourcearr(cur_indx, engIndx), cur_coef*fluid_eng_src*eng_src_fac); //pelelm
            }
          }
